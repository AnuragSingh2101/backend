...
const addVideoToPlaylist = asyncHandler(
  async (videoId, playlistId, req, next) => {
...
const updateVideo = asyncHandler(async (req, res, next) => {
  ...
  if (req.file) {
    ...
    await deleteFromCloudinary(
      oldThumbnail.thumbnail.split("/").pop().split(".")[0]
    );
  }
...
const watchHistory = asyncHandler(async (req, res, next) => {
  ...
  if (!user.watchHistory || user.watchHistory.length === 0) {
    return res
      .status(200)
      .json(new ApiResponse(200, [], "No videos in watch history"));
  }
  ...
  if (!videos || videos.length === 0) {
    return res
      .status(200)
      .json(new ApiResponse(200, [], "No videos found in watch history"));
  }
